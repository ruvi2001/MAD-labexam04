...
import com.example.todolistapp.adapter.NoteAdpater
...
class HomeFragment : Fragment(R.layout.fragment_home),SearchView.OnQueryTextListener,MenuProvider {
    ...
    private fun setupHomeRecyclerView(){
        ...
        activity?.let {
            notesViewModel.getAllNotes().observe(viewLifecycleOwner){
                note-> noteAdpater.differ.submitList(note)
                updateUI(note)
            }
        }
    }
    private fun searchNote(query: String?){
        ...
        notesViewModel.searchNote(searchQuery).observe(this){
            list-> noteAdpater.differ.submitList(list)
